...
// Node.js compatible version for the main process
 ...
 */
const compareRequestWithMock = (request, mock) => {
  const differences = [];
  const comparison = {
...
const findMatchingMock = (request, mocks) => {
  ...
  return mocks.find((mock) => {
    ...
    const normalizeUrl = (url) => {
      try {
        // Use Node.js URL class for the main process
        const urlObj = new URL(url, "http://localhost");
        let pathname = urlObj.pathname.replace(/\/$/, "") || "/";
        const searchParams = new URLSearchParams(urlObj.search);
        // Sort search params for consistent comparison
        const sortedParams = new URLSearchParams();
        [...searchParams.entries()].sort().forEach(([key, value]) => {
 ...
 */
const getDifferenceSummary = (comparison) => {
  if (!comparison.hasDifferences) {
